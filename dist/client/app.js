...
function monthOptions(selectedMonth) { return Array.from({ length: 36 }, (_, index) => index + 1).map((month) => `<option value="${month}" ${month === Number(selectedMonth) ? "selected" : ""}>${month} เดือน</option>`).join(""); }
...
function renderDynamicTables() {
  if (output.salaryLoanTable) output.salaryLoanTable.innerHTML = Object.entries(loanTypes)
    ...
    .join("");
  document.querySelector("#currentContractTable").innerHTML = [0,1,2].map((index) => `<tr data-current-contract-row="${index}"><td><strong>${index + 1}</strong></td><td><select class="table-select current-contract-type" data-current-index="${index}" aria-label="ประเภทเงินกู้ปัจจุบันสัญญาที่ ${index + 1}">${loanOptions("")}</select></td><td><input class="table-input current-contract-amount" data-current-index="${index}" type="text" inputmode="numeric" pattern="[0-9,]*" value="0" aria-label="ยอดเงินกู้ปัจจุบันสัญญาที่ ${index + 1}" /></td><td><input class="table-input current-contract-payment" data-current-index="${index}" type="text" inputmode="numeric" pattern="[0-9,]*" value="0" aria-label="ภาระผ่อนปัจจุบันสัญญาที่ ${index + 1}" /></td><td><span class="term-status empty" data-current-status="${index}">ยังไม่ระบุ</span></td></tr>`).join("");
...
function getCurrentContracts() { return [0,1,2].map((index) => ({ index, type: document.querySelector(`.current-contract-type[data-current-index="${index}"]`)?.value || "", amount: numberValue(document.querySelector(`.current-contract-amount[data-current-index="${index}"]`)), payment: numberValue(document.querySelector(`.current-contract-payment[data-current-index="${index}"]`)) })); }
function updateCurrentContractSection() { const contracts = getCurrentContracts(); const active = contracts.filter((contract) => contract.type || contract.amount > 0 || contract.payment > 0); const complete = active.filter((contract) => contract.type && contract.amount > 0 && contract.payment > 0); const totalAmount = complete.reduce((sum, contract) => sum + contract.amount, 0); const totalPayment = complete.reduce((sum, contract) => sum + contract.payment, 0); setText(document.querySelector("#currentContractCount"), `${complete.length} จาก 3 สัญญา`); setText(document.querySelector("#currentContractAmountTotal"), formatBaht(totalAmount)); setText(document.querySelector("#currentContractPaymentTotal"), `${formatBaht(totalPayment)}/เดือน`); contracts.forEach((contract) => { const status = document.querySelector(`[data-current-status="${contract.index}"]`); if (!status) return; const empty = !contract.type && contract.amount === 0 && contract.payment === 0; const valid = Boolean(contract.type && contract.amount > 0 && contract.payment > 0); status.className = `term-status ${empty ? "empty" : valid ? "ok" : "bad"}`; status.textContent = empty ? "ยังไม่ระบุ" : valid ? "กรอกครบแล้ว" : "กรุณากรอกให้ครบ"; }); const conflict = document.querySelector("#currentDebtConflict"); if (active.length === 0) { conflict.hidden = true; if (currentDebtMode === "contracts") { currentDebtMode = "manual"; fields.currentMonthlyPayment.readOnly = false; fields.currentMonthlyPayment.value = savedManualMonthlyPayment; } } else if (currentDebtMode === "manual" && numberValue(fields.currentMonthlyPayment) > 0) { conflict.hidden = false; setText(document.querySelector("#currentDebtConflictText"), `ช่องภาระผ่อนปัจจุบันมีค่า ${formatBaht(numberValue(fields.currentMonthlyPayment))} ขณะที่ผลรวมจากสัญญาที่กรอกครบคือ ${formatBaht(totalPayment)} กรุณาเลือกแหล่งข้อมูลเพื่อป้องกันการนับซ้ำ`); } else { conflict.hidden = true; currentDebtMode = "contracts"; fields.currentMonthlyPayment.readOnly = true; fields.currentMonthlyPayment.value = totalPayment; } const usingContracts = currentDebtMode === "contracts"; setText(document.querySelector("#currentContractSource"), usingContracts ? `กำลังใช้ผลรวมจากสัญญาปัจจุบัน ${complete.length} สัญญา เป็นภาระผ่อนปัจจุบันต่อเดือน` : "ขณะนี้ใช้ค่าจากช่อง “ภาระผ่อนปัจจุบันต่อเดือน”"); return { contracts, complete, totalAmount, totalPayment }; }
...
function buildReportViewModel(data) {
  ...
  const currentContractRows = data.currentContracts.filter((contract) => contract.type && contract.amount > 0 && contract.payment > 0).map((contract) => ({ ...contract, loanType: loanTypes[contract.type] }));
...
function calculate() {
  ...
  if (useCalculationButton) { useCalculationButton.disabled = !canUseFirstContract; useCalculationButton.title = canUseFirstContract ? "ดึงข้อมูลสัญญาที่ 1 ไปยังแบบคำขอ" : "ต้องให้สัญญาที่ 1 ผ่านและกู้ได้เต็มยอดก่อน"; }
...
function buildReportSummary(options = {}) {
  ...
  const currentContractRows = report.currentContractRows
    .map(({ index, type, loanType, amount, payment }) => `<tr>
      <td><strong>สัญญาที่ ${index + 1}</strong></td><td><strong>${type}</strong></td><td>${escapeHtml(loanType.label)}</td>
    ...
    <div class="report-print-area">
      ...
      <div class="current-loan-summary report-card">
        <div class="table-head">
          <h3>สัญญาเงินกู้ปัจจุบัน</h3><span>แสดงยอดเงินกู้ปัจจุบันและภาระผ่อนปัจจุบันของแต่ละสัญญา</span>
        </div>
        <div class="loan-table-wrap">
          <table class="loan-table compact-table current-loan-summary-table">
            <thead>
              <tr>
                <th>สัญญา</th><th>ประเภท</th>
                <th>รายละเอียด</th>
...
function resetForm() {
  ...
  currentDebtMode = "manual"; savedManualMonthlyPayment = 0; fields.currentMonthlyPayment.readOnly = false;
...
document.querySelector("#loanForm").addEventListener("input", (event) => {
  resetReportOutput();
  if (event.target.matches(".current-contract-amount,.current-contract-payment")) { event.target.value = formatLoanAmount(event.target.value) || "0"; updateCurrentContractSection(); scheduleCalculate(); return; }
  if (event.target.matches(".requested-loan-amount")) event.target.value = formatLoanAmount(event.target.value) || "0";
...
const PDF_FIELD_MAP = Object.freeze({ writtenAt: "เขียนที่", applicationDate: "วันที่", borrower: "ผู้กู้", position: "ตําแหน่ง", department: "สังกัด", amount: "จำนวนเงินที่ขอกู้", amountText: "จำนวนเงินเป็นตัวอักษร", months: "ระยะเวลาผ่อน (เดือน)", monthlyPayment: "ยอดผ่อนรายเดือน", receiveDay: "วันที่รับเงิน", receiveMonth: "เดือนที่รับเงิน", finalMonth: "เดือนที่ชําระเงินกู้งวดสุดท้าย", receiveYear: "ปี พ.ศ. ที่รับเงิน", finalYear: "ปี พ.ศ. ที่ชําระเงินกู้งวดสุดท้าย", guarantor1: "ผู้ค้ำ ที่1", guarantor2: "ผู้ค้ำ ที่2" });
...
const getApplicationReviewData = () => { const purpose = applicationPanel.querySelector('[name="applicationPurpose"]:checked')?.value || ""; const attachments = Array.from(applicationPanel.querySelectorAll("[data-application-attachment]:checked")).map((input) => input.closest("label")?.innerText.trim()).filter(Boolean); return [["ชื่อผู้กู้", fullApplicationName('select[name="applicationPrefix"]', "#applicationBorrowerName")], ["ตำแหน่ง", applicationFieldValue('[name="applicationPosition"]')], ["สังกัด", applicationFieldValue('[name="applicationDepartment"]')], ["จำนวนเงินที่ขอกู้", `${formatLoanAmount(applicationFieldValue("#applicationAmount"))} บาท`], ["จำนวนเงินเป็นตัวอักษร", applicationFieldValue("#applicationAmountText")], ["วัตถุประสงค์", purposeLabels[purpose] || "-"], ["ระยะเวลาผ่อน", `${applicationFieldValue("#applicationMonths")} เดือน`], ["ค่างวดต่อเดือน", `${Number(applicationFieldValue("#applicationMonthlyPayment")).toLocaleString("th-TH")} บาท`], ["เอกสารแนบ", attachments.join(" • ")], ["ผู้ค้ำประกันคนที่ 1", fullApplicationName('select[name="applicationGuarantor1Prefix"]', '[name="applicationGuarantor1"]')], ["ผู้ค้ำประกันคนที่ 2", fullApplicationName('select[name="applicationGuarantor2Prefix"]', '[name="applicationGuarantor2"]')], ["วันที่รับเงิน", [applicationFieldValue("#applicationReceiveDay"), thaiMonthNames[Number(applicationFieldValue("#applicationReceiveMonth")) - 1], applicationFieldValue("#applicationReceiveYear")].filter(Boolean).join(" ") || "ไม่ได้ระบุ"], ["วันครบกำหนดงวดสุดท้าย", applicationFieldValue("#applicationFinalDueDate") || "ยังไม่คำนวณ"]]; };
const openApplicationReview = () => { const invalidFields = validateApplication(); if (invalidFields.length) { alert(`กรุณาตรวจสอบข้อมูลที่จำเป็น ${invalidFields.length} รายการ โดยระบบเลื่อนไปยังช่องแรกที่ต้องแก้แล้ว`); return; } const dialog = document.querySelector("#applicationReviewDialog"); const content = document.querySelector("#applicationReviewContent"); content.replaceChildren(...getApplicationReviewData().map(([label, value]) => { const item = document.createElement("div"); const title = document.createElement("span"); const detail = document.createElement("strong"); title.textContent = label; detail.textContent = value; item.append(title, detail); return item; })); if (typeof dialog.showModal === "function") dialog.showModal(); else dialog.setAttribute("open", ""); };
...
const missingPdfFields = (form) => { const available = new Set(form.getFields().map((field) => field.getName())); return [...Object.values(PDF_FIELD_MAP), ...PDF_CONTROL_FIELDS].filter((name) => !available.has(name)); };
