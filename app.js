...
document.querySelector("#applicationPdfButton").addEventListener("click", fillOriginalPdf);
...
document.querySelector('select[name="applicationPrefix"]')?.addEventListener("change", (event) => { const borrowerPrefix = document.querySelector("#borrowerPrefix"); if (borrowerPrefix) borrowerPrefix.value = event.target.value; });const pdfRectangles={docNo:[399.06,692.9,443.156,706.546],writtenAt:[367.174,669.86,535.226,683.506],requestDate:[356.815,646.94,535.183,660.586],borrower:[212.442,573.86,369.871,587.506],position:[405.414,573.86,533.122,587.506],department:[81.725,550.91,368.112,564.556],amount:[88.293,527.87,207.052,541.516],amountText:[232.301,527.87,427.014,541.516],months:[177.638,389.87,257.709,403.516],monthly:[355.943,389.87,499.486,403.516],guarantor1:[66.328,260.91,252.23,274.527],guarantor2:[330.562,260.85,531.69,274.496],receiptName:[70.464,192.11,196.476,202.37],receiptAmount:[253.704,192.11,325.416,202.37],receiptAmountText:[344.83,192.11,459.322,202.37]};
const pdfAttachmentRects=[[66.744,365.69,73.908,371.591],[66.744,355.61,73.908,361.511],[66.744,345.65,73.908,351.551],[66.744,335.69,73.908,341.591],[66.744,325.61,73.908,331.591],[66.744,315.65,73.908,321.551],[66.744,305.69,73.908,311.591]];
const pdfPurposeRects=[[135.928,506.024,149.012,516.01],[135.928,460.087,149.012,470.074],[137.018,437.029,149.012,447.015],[135.928,413.97,149.012,423.956],[135.928,482.965,149.012,492.951]];
const pdfBytesFromBase64=(base64)=>{const binary=atob(base64);const bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i+=1)bytes[i]=binary.charCodeAt(i);return bytes};
const pdfFieldValue=(selector)=>document.querySelector(selector)?.value?.trim()||"";
const pdfDateParts=(value)=>{const match=String(value||"").match(/(\d{1,2})\s*[\/-]\s*(\d{1,2})\s*[\/-]\s*(\d{4})/);if(!match)return null;let year=Number(match[3]);if(year<2400)year+=543;return{day:match[1],month:match[2],year:String(year)}};
const pdfDrawTextImage=async(page,pdfDoc,rect,value,size=9)=>{if(!value)return;const maxWidth=rect[2]-rect[0]-3,boxHeight=rect[3]-rect[1]-2,scale=4,canvas=document.createElement("canvas");canvas.width=Math.max(32,Math.ceil(maxWidth*scale));canvas.height=Math.max(32,Math.ceil(boxHeight*scale));const ctx=canvas.getContext("2d");let fontSize=size*scale;ctx.textBaseline="middle";while(fontSize>22){ctx.font=`${fontSize}px Tahoma, sans-serif`;if(ctx.measureText(value).width<=canvas.width-4)break;fontSize-=1}ctx.font=`${fontSize}px Tahoma, sans-serif`;ctx.fillStyle="#000";ctx.fillText(value,2,canvas.height/2);const image=await pdfDoc.embedPng(canvas.toDataURL("image/png"));const ratio=(ctx.measureText(value).width+2)/scale;page.drawImage(image,{x:rect[0]+1,y:rect[1]+1,width:Math.min(maxWidth,ratio),height:boxHeight})};
const pdfDrawTick=(page,rect)=>{const x=rect[0]+1.5,y=rect[1]+1.2,w=rect[2]-rect[0],h=rect[3]-rect[1],color=window.PDFLib.rgb(0,0,0);page.drawLine({start:{x,y:y+h*.42},end:{x:x+w*.32,y},thickness:1.2,color});page.drawLine({start:{x:x+w*.32,y},end:{x:x+w,y:y+h},thickness:1.2,color})};
const pdfPurposeIndex={education:0,housing:1,equipment:2,emergency:3,retirement:4},pdfAttachmentIndex={"1.1":0,"1.2":1,"1.3":2,"1.4":3,"1.5":4,"1.6":5,"1.7":6};
async function fillOriginalPdf(){const button=document.querySelector("#applicationPdfButton"),oldText=button.textContent;button.disabled=true;button.textContent="กำลังสร้าง PDF...";try{const{PDFDocument,PDFName}=window.PDFLib;const pdfDoc=await PDFDocument.load(pdfBytesFromBase64(window.PDF_TEMPLATE_BASE64),{updateMetadata:false});const page=pdfDoc.getPages()[0],prefix=pdfFieldValue('select[name="applicationPrefix"]'),borrower=`${prefix}${pdfFieldValue("#applicationBorrowerName")}`,amount=pdfFieldValue("#applicationAmount"),amountText=pdfFieldValue("#applicationAmountText")||thaiAmountText(amount),guarantor1=`${pdfFieldValue('select[name="applicationGuarantor1Prefix"]')}${pdfFieldValue('input[name="applicationGuarantor1"]')}`,guarantor2=`${pdfFieldValue('select[name="applicationGuarantor2Prefix"]')}${pdfFieldValue('input[name="applicationGuarantor2"]')}`;const entries=[[pdfRectangles.docNo,pdfFieldValue('input[name="applicationDocNo"]')],[pdfRectangles.writtenAt,pdfFieldValue('input[name="applicationWrittenAt"]')],[pdfRectangles.requestDate,pdfFieldValue('input[name="applicationDate"]')],[pdfRectangles.borrower,borrower],[pdfRectangles.position,pdfFieldValue('input[name="applicationPosition"]')],[pdfRectangles.department,pdfFieldValue('select[name="applicationDepartment"]')],[pdfRectangles.amount,amount],[pdfRectangles.amountText,amountText],[pdfRectangles.months,pdfFieldValue("#applicationMonths")],[pdfRectangles.monthly,pdfFieldValue("#applicationMonthlyPayment")],[pdfRectangles.guarantor1,guarantor1],[pdfRectangles.guarantor2,guarantor2],[pdfRectangles.receiptName,borrower],[pdfRectangles.receiptAmount,amount],[pdfRectangles.receiptAmountText,amountText]];for(const[rect,value]of entries)await pdfDrawTextImage(page,pdfDoc,rect,value);const purpose=applicationPanel.querySelector('input[name="applicationPurpose"]:checked')?.value;if(purpose!==undefined)pdfDrawTick(page,pdfPurposeRects[pdfPurposeIndex[purpose]]);applicationPanel.querySelectorAll("[data-application-attachment]:checked").forEach((box)=>pdfDrawTick(page,pdfAttachmentRects[pdfAttachmentIndex[box.dataset.applicationAttachment]]));const receiveDate=pdfDateParts(pdfFieldValue('input[name="applicationReceiveDate"]'));if(receiveDate){await pdfDrawTextImage(page,pdfDoc,[71.256,175.04,100.296,185.3],receiveDate.day,8);await pdfDrawTextImage(page,pdfDoc,[117.048,175.04,161.828,185.3],receiveDate.month,8);await pdfDrawTextImage(page,pdfDoc,[173.948,175.04,206.912,185.3],receiveDate.year,8)}const finalDate=pdfDateParts(pdfFieldValue('input[name="applicationFinalDueDate"]'));if(finalDate){await pdfDrawTextImage(page,pdfDoc,[361.874,175.04,406.442,185.3],finalDate.month,8);await pdfDrawTextImage(page,pdfDoc,[418.55,175.04,472.838,185.3],finalDate.year,8)}page.node.delete(PDFName.of("Annots"));const bytes=await pdfDoc.save({useObjectStreams:false}),link=document.createElement("a");link.href=URL.createObjectURL(new Blob([bytes],{type:"application/pdf"}));link.download="แบบคำขอกู้ยืมเงินสวัสดิการทางวิชาการ-กรอกแล้ว.pdf";link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)}catch(error){console.error(error);alert("ไม่สามารถสร้าง PDF จากแบบฟอร์มต้นฉบับได้ กรุณาตรวจสอบข้อมูลแล้วลองอีกครั้ง")}finally{button.disabled=false;button.textContent=oldText}}
